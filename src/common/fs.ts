...
import { open_f } from "./lib/libraries/libsystem_kernel.js";
...
function readdir(path: string, max = 500, folderOnly = false): File[] {
  const list = successful((pError) =>
    NSFileManager.defaultManager().contentsOfDirectoryAtPath_error_(
      path,
      pError,
    ),
  );
  ...
  for (let i = 0, j = 0; i < count; i++) {
    ...
    NSFileManager.defaultManager().fileExistsAtPath_isDirectory_(
      absolute,
      pIsDir,
    );
    ...
    if (isFile && filename.toString().match(/^frida-([a-zA-z0-9]+)\.dylib$/))
      continue;
    if (j++ > max) break;
...
export function plist(path: string) {
  ...
  if (!info) throw new Error(`"${path}" is not valid plist format`);
  return valueOf(info);
}
...
export function write(path: string, text: string) {
  ...
  return successful((pError) =>
    NSString.stringWithString_(text).writeToFile_atomically_encoding_error_(
      path,
      NULL,
...
export async function text(path: string) {
  ...
  const fd = open_f(name, 0, 0) as number;
