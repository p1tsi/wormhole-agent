//Credits: https://gist.github.com/miticollo/aa27be66fd6c12fddd9079fa4f1967bf

import { libSystemC, snprintf, inet_ntop } from "./lib/libraries/libsystem_c.js";
import { libSystemInfo, getservbyport } from "./lib/libraries/libsystem_info.js";
import {
  libSystemKernel,
  proc_pidinfo,
  proc_pidfdinfo,
} from "./lib/libraries/libsystem_kernel.js";
import { libSystemMalloc, malloc } from "./lib/libraries/libsystem_malloc.js";
...
void get_file_descriptors(int pid, uint8_t type) {
    ...
    for (int i = 0; i < numberOfProcFDs; i++) {
        ...
        switch (finfo->proc_fdtype) {
                    ...
                    switch (socketInfo.psi.soi_kind) {
                        ...
                        case SOCKINFO_IN: {     // Type: UDP
                            ...
                            s = socketInfo.psi.soi_kind == SOCKINFO_TCP ?
                                &socketInfo.psi.soi_proto.pri_tcp.tcpsi_ini : &socketInfo.psi.soi_proto.pri_in;
            ...
            case PROX_FDTYPE_KQUEUE: {
                // TODO: missing body
                //error("PROX_FDTYPE_KQUEUE");
                break;
            }
            case PROX_FDTYPE_NETPOLICY: {
                // TODO: missing body
                //error("PROX_FDTYPE_NETPOLICY");
                break;
            }
            case PROX_FDTYPE_PSHM: {
                // TODO: missing body
                //error("PROX_FDTYPE_PSHM");
                break;
            }
            default: {
                //char str[100];
...
const cm: CModule = new CModule(code, {
  proc_pidinfo:
    Process.getModuleByName(libSystemKernel).getExportByName(proc_pidinfo),
  proc_pidfdinfo:
    Process.getModuleByName(libSystemMalloc).getExportByName(proc_pidfdinfo),
  malloc: Process.getModuleByName(libSystemMalloc).getExportByName(malloc),
  snprintf: Process.getModuleByName(libSystemC).getExportByName(snprintf),
  inet_ntop: Process.getModuleByName(libSystemC).getExportByName(inet_ntop),
  getservbyport:
    Process.getModuleByName(libSystemInfo).getExportByName(getservbyport),
  push_vnode,
  push_socket,
  error,
});

export function getFds(type: number) {
  new NativeFunction(cm.get_file_descriptors, "void", ["int64", "uint8"])(
    Process.id,
    type,
  );
  let retArr = Array.from(fds).reduce((o, [key, value]) => {
    o[key] = value;
    return o;
  }, {});
  fds.clear();
