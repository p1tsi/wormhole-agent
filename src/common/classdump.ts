...
} from "./lib/libraries/libobjc.A.js";
import { free_f } from "./lib/libraries/libsystem_malloc.js";


const { NSBundle, NSString } = ObjC.classes;
const copyClassNamesForImage = new NativeFunction(
  Process.getModuleByName(libObjCA).getExportByName(
    objc_copyClassNamesForImage,
  )!,
  "pointer",
  ["pointer", "pointer"],
...
const normalize = (path: string) =>
  NSString.stringWithString_(path)
    ...
    .toString();
const flattern = (array: any[]) =>
  array.reduce((sum, item) => sum.concat(item), []);
...
export function inspect(clazz: string) {
  ...
  while ((cls = cls.$superClass)) prototypeChain.unshift(cls.$className);
