const demangle = new NativeFunction(
  Process.getModuleByName('libc++abi.dylib').getExportByName('__cxa_demangle')!,
...
function swiftDemangle(name: string) {
  ...
  if (mod) {
    const demangle = new NativeFunction(mod.getExportByName('swift_demangle_getDemangledName')!, 'uint', ['pointer', 'pointer', 'uint'])
